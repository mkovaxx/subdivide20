...
QvBool QvDict::enter(unsigned long key, void* value) {
...
QvBool QvDict::find(unsigned long key, void*& value) const {
    QvDictEntry*& entry = findEntry(key);
...
QvDictEntry*& QvDict::findEntry(unsigned long key) const {
    QvDictEntry** entry;
...
QvBool QvDict::remove(unsigned long key) {
    QvDictEntry*& entry = findEntry(key);
    QvDictEntry* tmp;

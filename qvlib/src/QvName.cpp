...
const QvNameEntry* QvNameEntry::insert(const char* s) {
    unsigned long h = QvString::hash(s);
    unsigned long i;
    QvNameEntry* entry;
    QvNameEntry* head;
    ...
    if (entry == NULL) {
        ...
        if (len >= CHUNK_SIZE) {
            char* tmps = (char*)malloc(sizeof(char) * (strlen(s) + 1));
            strcpy(tmps, s);
            s = tmps;
        } else {

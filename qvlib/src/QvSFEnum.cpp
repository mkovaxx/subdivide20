...
QvBool QvSFEnum::findEnumName(int val, QvName& name) const {
    int i;
...
QvBool QvSFEnum::findEnumValue(const QvName& name, int& val) const {
    int i;
...
QvBool QvSFEnum::readValue(QvInput* in) {
    QvName n;

#ifdef DEBUG
    if (enumValues == NULL) {
        QvDebugError::post("QvSFEnum::readValue", "Enum values were never initialized");

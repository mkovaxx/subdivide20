...
void QvFieldData::addField(QvNode* defobj, const char* fieldName, const QvField* field) {
...
const QvName& QvFieldData::getFieldName(int index) const { return ((QvFieldEntry*)fields[index])->name; }

QvField* QvFieldData::getField(const QvNode* object, int index) const {

    return (QvField*)((char*)object + ((QvFieldEntry*)fields[index])->offset);
}

#include <ctype.h>
static QvName stripWhite(const char* name) {
    int firstchar = -1;
    int lastchar = -1;
    int lastwhite = -1;
...
void QvFieldData::addEnumValue(const char* typeNameArg, const char* valNameArg, int val) {
...
void QvFieldData::getEnumData(const char* typeNameArg, int& num, const int*& vals, const QvName*& names) {
...
QvBool QvFieldData::readFieldTypes(QvInput* in, QvNode* object) {
...
QvBool QvFieldData::read(QvInput* in, QvNode* object, QvBool errorOnUnknownField) const {
    ...
    while (TRUE) {
        ...
        if (!foundName) {
            if (errorOnUnknownField) {
                QvReadError::post(in, "Unknown field \"%s\"", fieldName.getString());
                return FALSE;
            } else {
...
QvBool QvFieldData::read(QvInput* in, QvNode* object, const QvName& fieldName, QvBool& foundName) const {

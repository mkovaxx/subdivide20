...
class QvString;
int operator==(const QvString& str, const char* s);
inline int operator==(const char* s, const QvString& str);
inline int operator==(const QvString& str1, const QvString& str2);
int operator!=(const QvString& str, const char* s);
inline int operator!=(const char* s, const QvString& str);
inline int operator!=(const QvString& str1, const QvString& str2);
...
class QvString {
  public:
    ...
    QvString(const char* str) {
        string = staticStorage;
        *this = str;
    }
    QvString(const QvString& str) {
        string = staticStorage;
        *this = str.string;
    }
    ~QvString();
    unsigned long hash() { return QvString::hash(string); }
    int getLength() const { return strlen(string); }
    void makeEmpty(QvBool freeOld = TRUE);
    const char* getString() const { return string; }
    QvString& operator=(const char* str);
    QvString& operator=(const QvString& str) { return (*this = str.string); }
    QvString& operator+=(const char* str);
    int operator!() const { return (string[0] == '\0'); }
    friend int operator==(const QvString& str, const char* s);

    friend int operator==(const char* s, const QvString& str) { return (str == s); }

    friend int operator==(const QvString& str1, const QvString& str2) { return (str1 == str2.string); }
    friend int operator!=(const QvString& str, const char* s);
...
class QvNameEntry {
  public:
    ...
    QvBool isEqual(const char* s) const { return (string[0] == s[0] && !strcmp(string, s)); }
  ...
  private:
    ...
    QvNameEntry(const char* s, unsigned long h, QvNameEntry* n) {
        string = s;
        hashValue = h;
        next = n;
    }
...
class QvName;
inline int operator==(const QvName& n, const char* s);
inline int operator==(const char* s, const QvName& n);
inline int operator==(const QvName& n1, const QvName& n2);
inline int operator!=(const QvName& n, const char* s);
inline int operator!=(const char* s, const QvName& n);
inline int operator!=(const QvName& n1, const QvName& n2);
...
class QvName {
  public:
    ...
    friend int operator==(const QvName& n, const char* s) { return n.entry->isEqual(s); }
    friend int operator==(const char* s, const QvName& n) { return n.entry->isEqual(s); }

    friend int operator==(const QvName& n1, const QvName& n2) { return n1.entry == n2.entry; }
    friend int operator!=(const QvName& n, const char* s) { return !n.entry->isEqual(s); }
    friend int operator!=(const char* s, const QvName& n) { return !n.entry->isEqual(s); }

...
class HMatrix {
  ...
  public:
    ...
    static HMatrix Identity() { return HMatrix().setIdentity(); }

    HMatrix() { setIdentity(); }

    HMatrix(const HMatrix& mat) { memcpy(_m, mat._m, 16 * sizeof(double)); }

    HMatrix(float m00, float m01, float m02, float m03, float m10, float m11, float m12, float m13, float m20,
    ...
    operator double*() { return _m; }

    operator const double*() const { return _m; }

    explicit HMatrix(double* m) { memcpy(_m, m, 16 * sizeof(double)); }

    HMatrix& setTranslation(const CVec3f& trans) {
        setIdentity();
        (*this)(0, 3) = trans.x();
        (*this)(1, 3) = trans.y();
        (*this)(2, 3) = trans.z();
    ...
    static HMatrix Translation(const CVec3f& trans) { return HMatrix().setTranslation(trans); }

    double operator()(int i, int j) const { return _m[4 * j + i]; }

    double& operator()(int i, int j) { return _m[4 * j + i]; }

    HMatrix& operator=(const HMatrix& mat) {
        memcpy(_m, mat._m, 16 * sizeof(double));
        return *this;
    }
    CVec3f col(int i) { return CVec3f(_m[4 * i], _m[4 * i + 1], _m[4 * i + 2]); }

    CVec3f row(int i) { return CVec3f(_m[i], _m[i + 4], _m[i + 8]); }

    // create a rotation matrix for

    static HMatrix Rotation(const CVec3f& v, float a) {
        CVec3f u = v.dir();
        ...
        HMatrix U = HMatrix(u1 * u1, u1 * u2, u1 * u3, 0, u2 * u1, u2 * u2, u2 * u3, 0, u3 * u1, u3 * u2, u3 * u3, 0, 0,
                            0, 0, 1);

        HMatrix S = HMatrix(0, -u3, u2, 0, u3, 0, -u1, 0, -u2, u1, 0, 0, 0, 0, 0, 1);
    ...
    CVec3f operator*(const CVec3f& v) const {
        ...
        return CVec3f(v.x() * M(0, 0) + v.y() * M(0, 1) + v.z() * M(0, 2) + M(0, 3),
    ...
    HMatrix operator*(float s) const {
        const HMatrix& M = *this;
        return HMatrix(s * M(0, 0), s * M(0, 1), s * M(0, 2), s * M(0, 3), s * M(1, 0), s * M(1, 1), s * M(1, 2),
                       s * M(1, 3), s * M(2, 0), s * M(2, 1), s * M(2, 2), s * M(2, 3), s * M(3, 0), s * M(3, 1),
                       s * M(3, 2), s * M(3, 3));
    }

    HMatrix operator*(const HMatrix& M2) const {
        const HMatrix& M1 = *this;
        return HMatrix(M1(0, 0) * M2(0, 0) + M1(0, 1) * M2(1, 0) + M1(0, 2) * M2(2, 0) + M1(0, 3) * M2(3, 0),
    ...
    HMatrix operator+(const HMatrix& M2) const {
        const HMatrix& M1 = *this;
        return HMatrix(M1(0, 0) + M2(0, 0), M1(0, 1) + M2(0, 1), M1(0, 2) + M2(0, 2), M1(0, 3) + M2(0, 3),

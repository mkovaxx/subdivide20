...
class SectorInfo {
  public:
    ...
    SectorInfo()
        : _refCount(0), _sectorTag(NOTAG_SECTOR), _normal(0), _modifiedNormal(0), _modifiedNormalT(0),
          _modifiedFlatness(0), _theta(-1) {}

    virtual ~SectorInfo() {}

    bool isDefault() const {
        return (_sectorTag == NOTAG_SECTOR) && (_modifiedNormalT == 0) && (_modifiedFlatness == 0);
    }
    ...
    void setModifiedFlatness(float s) { _modifiedFlatness = s; }

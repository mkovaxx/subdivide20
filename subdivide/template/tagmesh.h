...
private:
  ...
  void applyVertTag(const TagFlatMesh& flatMesh,
  ...
  void setVertTag(TagFlatMesh* flatMesh,
		  vector<int>& index,
		  VertexTagType tag) const;
...
template<class Face>
void TagMeshTp<Face>::applyVertTag(const TagFlatMesh& m,
				   const vector<int>& tagVertVec, 
                                   VertexTagType tag,
				   const EMapType& _eMap) {
...
template<class Face>
void TagMeshTp<Face>::applyCreaseEdge(const TagFlatMesh& m,
                                      const EdgeMapType& _edgeMap) {
...
template<class Face>
void TagMeshTp<Face>::applySector(const TagFlatMesh& flatMesh,
				  const EdgeMapType& _edgeMap) {
  ...
  for(uint i = 0; i < flatMesh.sectorInfoVec.size(); ++i) {
    ...
    if((f->headVert(f->nextEno(fe)) == sv) ||
       (f->tailVert(f->prevEno(fe)) == ppv)) {
      fv = f->headVno(fe);
    } else {
      EnoType ne;
      f = f->neighbor(fe, ne);
      assert((f->headVert(f->nextEno(ne)) == sv) ||
	     (f->tailVert(f->prevEno(ne)) == ppv));
      fv = f->headVno(ne);
      fe = ne;
    }
...
template<class Face>
void TagMeshTp<Face>::setSector(TagFlatMesh* m) const {
  ...
  for(it = faceSet().begin(); it != faceSet().end(); ++it) {
    ...
    for(i = 0; i < (*it)->noVtx(); ++i, e = (*it)->nextEno(e)) {
      ...
      if(((*it)->sectorInfo((v)) != 0) && 
	 !((*it)->sectorInfo(v)->isDefault())) {
...
template<class Face>
void TagMeshTp<Face>::setCreaseEdge(TagFlatMesh* m, const VertexMapType& _vertexMap) const {
  typename set<FaceType*>::const_iterator it;
  for(it = faceSet().begin(); it != faceSet().end(); ++it) {
    for(EnoType e = 1; e < (*it)->noVtx()+1; ++e) {
...
template<class Face>
void TagMeshTp<Face>::setVertTag(TagFlatMesh*, 
				 vector<int>& tagVertVec, 
				 VertexTagType tag) const {

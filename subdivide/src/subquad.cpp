// -*- Mode: c++ -*-
// $Id: subquad.cpp,v 1.6 2001/02/07 05:29:40 biermann Exp $
// $Source: /tools/cvs-repos/sig00code/nyuSub/src/subquad.cpp,v $
...
cvec3f Quad::computeVertexPoint(EnoType e, int d) {
    ...
    if (hasHeadPos(e, d + 1)) {
        ...
        if (headVert(e)->isSpecial()) {
            ...
            tmp = (1.0 - beta1 - beta2) * headPos(e, d) + beta1 / k * tmpposE + beta2 / k * tmpposF;
        }
...
void Quad::computeEdgeNormal(EnoType e, int d) {
    ...
    if (!mf->hasNormal(mf->headVno(me))) {
        ...
        cvec3f cp(computeEdgePoint(e, d)), v0(headPos(e, d + 1)), v1(computeFacePoint(d)), v2(tailPos(e, d + 1)),
            f0(computeEdgePoint(nextEno(e), d)), f1(computeEdgePoint(prevEno(e), d));

        if (edgeTag(e) == Face::CREASE_EDGE) {
            a0 = 2.0 / 3.0f * cp + 1 / 6.0 * (v0 + v2);
            a1 = -2.0 / 3.0f * cp - 1 / 6.0 * v0 + 2 / 3.0 * v1 - 1 / 6.0 * v2 + 1 / 6.0f * f0 + 1 / 6.0f * f1;
            a2 = 0.0f * cp + 0.5f * v0 + 0.0 * v1 - 0.5 * v2;
        } else {
            EnoType ne;
            Face* nf = neighbor(e, ne);
            cvec3f f2(nf->computeEdgePoint(nf->prevEno(ne), d)), v3(nf->computeFacePoint(d)),
                f3(nf->computeEdgePoint(nf->nextEno(ne), d));

            a0 = 4.0f / (4.0f + 5.0f) * cp + 1.0f / 4.0f * (v0 + v1 + v2 + v3) + 1.0f / 16.0f * (f0 + f1 + f2 + f3);

            a1 =
                0.0f * cp + 0.0f * v0 + 4.0f * v1 + 0.0 * v2 - 4.0f * v3 + 1.0f * f0 + 1.0f * f1 - 1.0 * f2 - 1.0f * f3;

            a2 =
                0.0f * cp + 4.0f * v0 + 0.0f * v1 - 4.0 * v2 + 0.0f * v3 + 1.0f * f0 - 1.0f * f1 - 1.0 * f2 + 1.0f * f3;
        }
...
void Quad::computeFaceNormal(int d) {
    ...
    if (noVtx() != 4) {
        ...
        cvec3f v0(midPos(1, d)), f0(tailPos(1, d + 1)), v1(midPos(2, d)), f1(tailPos(2, d + 1)), v2(midPos(3, d)),
            f2(tailPos(3, d + 1)), v3(midPos(4, d)), f3(tailPos(4, d + 1));

        cvec3f a0 = 4.0f / (4.0f + 5.0f) * cp + 1.0f / 4.0f * (v0 + v1 + v2 + v3) + 1.0f / 16.0f * (f0 + f1 + f2 + f3);

        cvec3f a1 = 0.0f * cp + 4.0f * (0.0f * v0 + 1.0f * v1 + 0.0f * v2 + (-1.0f) * v3) +
                    1.0f * (1.0f * f0 + 1.0f * f1 + (-1.0f) * f2 + (-1.0f) * f3);

        cvec3f a2 = 0.0f * cp + 4.0f * (1.0f * v0 + 0.0f * v1 + (-1.0f) * v2 + (0.0f) * v3) +
                    1.0f * (1.0f * f0 + (-1.0f) * f1 + (-1.0f) * f2 + (1.0f) * f3);
            ...
            if (this->headVert(e)->isSpecial()) {
                ...
                if (si && (si->hasModifiedNormal() || si->hasModifiedFlatness())) {
                    ...
                    if ((si = sectorInfo(headVno(e))) && si->hasModifiedNormal()) {
                        ...
                        res = (subdiv.ring().isClosed()) ? subdiv.modifyClosedNormal(res, x1, x2)
                                                         : subdiv.modifyOpenNormal(res, x1, x2);
                    }
                    ++relevantCnt;
                    relevantPos += res;
                }
...
cvec3f Quad::computeEdgePoint(EnoType e, int d) {
    ...
    if (hasMidPos(e, d)) {
        ...
        if (headVert(e)->isSpecial() || tailVert(e)->isSpecial()) {
            ...
            Face* nf = neighbor(e, ne);
            assert(nf);
            tmp = 1 / 4.0f * (headPos(e, d) + tailPos(e, d) + computeFacePoint(d) + nf->computeFacePoint(d));
        }
        setMidPos(e, d, tmp);
        return tmp;
    }
...
template <class QuadRing> CreaseQuadRuleTable QuadRuleTableTp<QuadRing>::_creaseRuleTable;
template <class QuadRing> InteriorQuadRuleTable QuadRuleTableTp<QuadRing>::_interiorRuleTable;
template <class QuadRing> ConvexQuadRuleTable QuadRuleTableTp<QuadRing>::_convexRuleTable;
template <class QuadRing> ConcaveQuadRuleTable QuadRuleTableTp<QuadRing>::_concaveRuleTable;

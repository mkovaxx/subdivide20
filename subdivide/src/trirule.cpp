// -*- Mode: c++ -*-
// $Id: trirule.cpp,v 1.4 2000/04/30 07:35:47 dzorin Exp $
// $Source: /tools/cvs-repos/sig00code/nyuSub/src/trirule.cpp,v $
...
static void computeBoundaryParams(float k, float& thetak, float& delta, float& gamma, float& a, float& b, float& theta,
                                  float& sigma1, float& sigma2, float& sigma3, float th = 0);
...
void InteriorTriRule::computeLambda2() { _lambda2 = 3 / 8.0 + 1 / 4.0 * cos(2 * M_PI / (float)k); }
...
void CreaseTriRule::computeL1() {
    ...
    if (k == 1) {
        ...
        computeBoundaryParams(k, thetak, delta, gamma, a, b, theta, sigma1, sigma2, sigma3);
...
void ConcaveTriRule::computeLambda2() { _lambda2 = 0.5; }
...
void ConvexTriRule::computeLambda2() { _lambda2 = 0.5; }
...
void computeBoundaryParams(float k, float& thetak, float& delta, float& gamma, float& a, float& b, float& theta,
    ...
    sigma2 = (cos(2.0f * thetak) + 1.0f + 2.0f * cos(thetak)) / (sin(thetak) + sin(2.0f * thetak));
